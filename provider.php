...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $insert_query = "INSERT INTO providers(name, email, tel, cnpj, cep, city) 
    VALUES('$name', '$email', '$tel', '$cnpj', '$cep', '$city')";
...
//LIST
$select_list = "SELECT * FROM providers";
$result_select = mysqli_query($conn, $select_list);

//SEARCH
if (!empty($_GET['search'])) {
    $data = $_GET['search'];
    $select_search = "SELECT * FROM providers WHERE 
    id_provider LIKE '%$data%' 
    OR name LIKE '%$data%' 
    OR email LIKE '%$data%'";
    $result_select = mysqli_query($conn, $select_search);
} else {
    $select_search = "SELECT * FROM providers ORDER BY id_provider DESC";
}
...
if (!empty($_GET['id'])) {
    ...
    if ($result_select->num_rows > 0) {
        $delete = "DELETE FROM providers WHERE id_provider=$id";
...
<body>
      ...
      <main class="main-container">
                              ...
                              <div class="form-row">
                                 ...
                                 <div class="col">
                                    <input type="email" class="form-control" id="email" name="email"
                                       placeholder="E-mail" pattern="[a-z0-9._%+\-]+@[a-z0-9.\-]+\.[a-z]{2,}$" autocomplete="off" required>
                                 </div>
                              </div><br>
                              <div class="form-row">
                                 <div class="col">
         ...
         <div class="box-table">
            <table class="table">
               ...
               <tbody class="">
                  <?php
                        while ($provider_data = mysqli_fetch_assoc($result_select)) { ?>
                  <tr>
                     <td><?php echo $provider_data['id_provider']; ?></td>
                     <td><?php echo $provider_data['name']; ?></td>
                     <td><?php echo $provider_data['email']; ?></td>
                     <td><?php echo $provider_data['tel']; ?></td>
                     <td><?php echo $provider_data['cnpj']; ?></td>
                     <td scope="col" style="display:flex;">
                        <div>
                           <?php echo "<a href='provider.php?id=$provider_data[id_provider]'>" ?>
                     ...
                     <td scope="col">
                        ...
                        <?php echo "<a href='edits/edit_provider.php?id=$provider_data[id_provider]'>" ?>
   ...
   function searchData() {
      window.location = 'provider.php?search=' + search.value;
   }
   </script>
